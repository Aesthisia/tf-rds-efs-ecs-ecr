...
resource "aws_efs_file_system" "confluence-data" {
  creation_token = "es-persistent-data-confluence"
...
resource "aws_efs_mount_target" "confluence" {
  count  = "2"
  file_system_id = "${aws_efs_file_system.confluence-data.id}"
